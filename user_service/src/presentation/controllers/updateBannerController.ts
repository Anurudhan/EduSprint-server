...
export const updateBannerController = (dependencies: IDependencies) => {
	const {
		useCases: {updateBannerUseCase },
	} = dependencies;
	return async (req: Request, res: Response, next: NextFunction) => {
		try {
			const data = req.body
			const result = await updateBannerUseCase(dependencies).execute(data);
